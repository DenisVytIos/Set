...
// Упорядочивает пространство в прямоугольнике в сетке ячеек.
// Все ячейки будут одинакового размера.
// Если сетка не заполняет предоставленный кадр от края до края
// тогда он будет центрировать сетку ячеек в предоставленном кадре.
// Если вы хотите расстояние между ячейками в сетке, просто вставьте кадр каждой ячейки.
//
// Как он размещает ячейки, определяется свойством layout:
// Компоновка может быть сделана путем (a) фиксирования размера ячейки
// (Сетка создаст столько строк и столбцов, сколько поместится)
// Или (б) фиксируем количество строк и столбцов
// (Сетка сделает ячейки максимально большими)
// Или (c) обеспечение определенного соотношения сторон (ширина и высота) для каждой ячейки
// (Сетка подгонит ячейки cellCount, сделав ячейки максимально большими)
// (вы должны установить переменную cellCount для макета aspectRatio, чтобы знать, что делать)
//
// Ограничительный прямоугольник ячейки в сетке получается по нижнему индексу (например, grid [11] или grid [1,5]).
// Кортеж размеров будет содержать количество (вычисленных или указанных) строк и столбцов.
// Установка aspectRatio, размеры или cellSize, может изменить макет.
//
// Чтобы использовать, просто используйте инициализатор, чтобы выбрать стратегию размещения и установить фрейм.
// После создания сетки вы можете в любой момент изменить стратегию фрейма или макета
// (все остальные свойства будут немедленно обновлены).
import UIKit
